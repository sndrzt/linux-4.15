...
kmem_cache_t *scull_cache; /* declare one cache pointer: use it for all devices */
                ...
                loff_t *f_pos)
{
	...
	if (!dptr->data[s_pos]) {
		dptr->data[s_pos] = kmem_cache_alloc(scull_cache, GFP_KERNEL); /* Allocate a quantum using the memory cache */
		if (!dptr->data[s_pos])
			goto nomem;
...
int scull_init(void)
{
	...
	scull_cache = kmem_cache_create("scull", scull_quantum,
