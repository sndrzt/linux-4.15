/*  -*- C -*-
 * mmap.c -- memory mapping for the scull char module
 *
 ...
 * page required from the scull device and returns it to the
...
struct page *scull_vma_nopage(struct vm_area_struct *vma,
                                unsigned long address, int *type)
{
	...
	/*
	 * Now retrieve the scull device from the list,then the page.
...
int scull_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (scull_devices[iminor(inode)].order) /* refuse to map if order is not 0 */
		return -ENODEV;
